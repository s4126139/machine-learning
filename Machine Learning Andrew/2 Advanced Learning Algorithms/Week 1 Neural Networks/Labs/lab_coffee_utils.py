...
def sigmoid(z):
    """NumPy sigmoid used by both the TensorFlow and NumPy coffee labs."""
    z = np.clip(z, -500, 500)
...
def plt_prob(ax,fwb):
    ...
    for i in range(tmp_x0.shape[0]):
        for j in range(tmp_x1.shape[1]):
            ...
            # NumPy 2.x requires an explicit scalar conversion here.
            z[i,j] = np.asarray(fwb(x)).squeeze().item()
