...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        ...
        # Matplotlib 3.7+ renders CheckButtons with collections instead of the
        # legacy ``rectangles`` and ``lines`` artists. The modern widget is
        # already square, so the old aspect-ratio correction is unnecessary.
        if not hasattr(bcid, "rectangles") or not hasattr(bcid, "lines"):
            return
