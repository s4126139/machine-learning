...
def _shape_as_list(shape):
    """Support both TensorShape and tuple shapes across TensorFlow/Keras versions."""
...
def test_tower(target):
    ...
    for layer in target.layers:
        ...
        actual_shape = _shape_as_list(layer.output.shape)
        assert actual_shape == expected[i][1], \
            f"Wrong number of units in layer {i}. Expected {expected[i][1]} but got {actual_shape}"
